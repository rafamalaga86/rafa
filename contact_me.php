...
if ($_POST) {
    ...
    $to_Email = "rafael@thesoftwarefactory0"; //Replace with recipient email address
   
    // Check $_POST vars are set, exit if any missing
    if (!isset($_POST["userName"]) || !isset($_POST["userEmail"]) || !isset($_POST["userSubject"]) || !isset($_POST["userMessage"])) {
        $output = json_encode(array('type'=>'error', 'text' => $formEmpty));
        die($output);
    }
    ...
    if (!filter_var($_POST["userEmail"], FILTER_VALIDATE_EMAIL)) {
        $output = json_encode(array('type'=>'error', 'text' => $formInvalidEmail ));
        die($output);
    }
    ...
    if (!$sentMail) {
        $output = json_encode(array('type'=>'error', 'text' => $formError));
        die($output);
    } else {
        $output = json_encode(array('type'=>'message', 'text' => sprintf($formThanks, $_POST["userName"])));
        die($output);
    }
}
